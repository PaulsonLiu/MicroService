...
namespace iFramework.Util
{
    ...
    {
        // Fields
        private static string m_strFirstLetter;
        private static string m_strFullCode;
        private static string m_strFullCodeWithSpace;
        private static string m_strInput;
        private static string[] pyStr;
        private static int[] pyValue;

        private ChineseSpellHelper() { }
        ...
        /// <returns></returns>
        public static string GetFirstLetter(string strInput,params string[] Fields)
        {
            var theInput = strInput;
            if (Fields != null)
            {
            ...
            m_strInput = theInput;
            GetPyCode();
            if (m_strFirstLetter == null)
            {
                return "";
            }
            if (m_strFirstLetter.Length > 256)
            {
                m_strFirstLetter.Substring(0, 256);
            }
            return m_strFirstLetter;
        }
        ...
        /// <param name="strInput">源字符串</param>
        /// <returns></returns>
        public static string GetFullCode(string strInput)
        {
            m_strInput = strInput;
            GetPyCode();
            return m_strFullCode;
        }
        ...
        /// <param name="strInput">源字符串</param>
        /// <returns></returns>
        public static string GetFullCodeWithSpace(string strInput)
        {
            m_strInput = strInput;
            GetPyCode();
            return m_strFullCodeWithSpace;
        }


        private static bool GetPyCode()
        {
            m_strFullCode = "";
            m_strFullCodeWithSpace = "";
            m_strFirstLetter = "";
            byte[] buffer1 = new byte[2];
            int num1 = 0;
            int num2 = 0;
            int num3 = 0;
            char[] chArray1 = m_strInput.ToCharArray();
            for (int num4 = 0; num4 < chArray1.Length; num4++)
            {
                ...
                if (num2 <= 0x7f)
                {
                    m_strFullCode = m_strFullCode + chArray1[num4];
                    m_strFullCodeWithSpace = m_strFullCodeWithSpace + chArray1[num4];
                    m_strFullCodeWithSpace = m_strFullCodeWithSpace + " ";
                    m_strFirstLetter = m_strFirstLetter + chArray1[num4];
                }
                else
                {
                    num3 = buffer1[1];
                    ...
                    {
                        for (int num5 = ChineseSpellHelper.pyValue.Length - 1; num5 >= 0; num5--)
                        {
                            if (ChineseSpellHelper.pyValue[num5] <= num1)
                            {
                                m_strFullCode = m_strFullCode + ChineseSpellHelper.pyStr[num5];
                                m_strFullCodeWithSpace = m_strFullCodeWithSpace + ChineseSpellHelper.pyStr[num5];
                                m_strFullCodeWithSpace = m_strFullCodeWithSpace + " ";
                                m_strFirstLetter = m_strFirstLetter + ChineseSpellHelper.pyStr[num5].Substring(0, 1);
                                break;
                            }
                        }
                    }
        ...
        {
            ChineseSpellHelper.pyValue = new int[] { 
            ...
            ChineseSpellHelper.pyStr = new string[] { 
